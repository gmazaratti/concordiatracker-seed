...
export function TeacherAdminPage() {
  ...
  const { teachers, approveTeacher, invites, createInvite, accessRequests, setRequestStatus } =
    useTeacher()
  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-5 py-6 sm:px-6">
      ...
      <p className="mt-0.5 text-[13px] text-subtle">
        Originate invites and approve teachers — the two gates.
      </p>
      ...
      {/* Invites */}
      <Block title={`Invites · ${invites.length}`}>
        <ul className="flex flex-col gap-2">
          {invites.map((inv) => (
