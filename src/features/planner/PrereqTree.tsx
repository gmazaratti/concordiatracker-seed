          ...
          {suggestions.map((c) => (
            <li key={c.id}>
              ...
              >
                ...
                <span className="min-w-0 flex-1 truncate text-[12.5px] text-subtle">{c.title}</span>
              </button>
            </li>
          ))}
        </ul>
    ...
    <>
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          ...
          {node.repeated && (
            <span className="text-[10.5px] text-subtle">already shown above</span>
          )}
          {!node.course && !isRoot && (
      ...
      {node.children.map((child) => (
        <Branch key={`${child.code}-${child.depth}`} node={child} trusted={trusted} onOpen={onOpen} />
      ))}
    </>
  )
}
